...
sys.stdout = Logger(f'logs/log_vim_fashionmnist_scratch.txt')
...
CONFIG = {
    ...
    'warmup_epochs': 10,
    'repeated_aug': True,
    'img_size': 224, # Resizing 28x28 to 224x224
    'workers': 8,
...
# ==========================================
# 3. Data Preparation (Fashion MNIST)
# ==========================================
print("\n🔄 Preparing Fashion MNIST dataset...")

# Fashion MNIST Mean/Std (Grayscale)
# mean=0.2860, std=0.3530
# But we are converting to 3 channels to match model input.
# We can just replicate the grayscale channel to 3 channels.

transform_train = transforms.Compose([
    transforms.Resize((224, 224)), # Resize 28x28 -> 224x224
    transforms.Grayscale(num_output_channels=3), # Convert 1 channel to 3 channels
    transforms.RandomHorizontalFlip(),
    transforms.RandAugment(num_ops=2, magnitude=9), 
    transforms.ToTensor(),
    transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)), # Normalize to [-1, 1] roughly
...
# ==========================================
print("\n" + "="*60)
print("🚀 Starting Training of Vision Mamba on Fashion MNIST")
print("="*60 + "\n")

best_acc = 0.0

def save_checkpoint(model, acc, epoch, filename="vim_fashionmnist_best.pth"):
    if not os.path.exists("checkpoints_fashionmnist"):
        os.makedirs("checkpoints_fashionmnist")
    filepath = f"checkpoints_fashionmnist/{filename}"
    torch.save({
        'epoch': epoch,
...
for epoch in range(CONFIG['epochs']):
    ...
    if test_acc > best_acc:
        ...
        best_acc = test_acc
        save_checkpoint(model, best_acc, epoch+1, f"vim_fashionmnist_best_acc{best_acc:.2f}.pth")
