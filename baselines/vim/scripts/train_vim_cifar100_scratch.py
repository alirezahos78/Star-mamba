...
sys.stdout = Logger(f'logs/log_vim_cifar100_scratch.txt')
...
# ==========================================
# 1. Configuration & Hyperparameters (Matched to cifar.py)
# ==========================================
CONFIG = {
    'seed': 42,
    'batch_size': 128,
    ...
    'warmup_epochs': 10,
    'repeated_aug': True,
    'img_size': 224, # Kept at 224 for Vim-tiny
    'workers': 8,
...
# Calculate FLOPs - DISABLED due to incompatibility with Triton/Mamba kernels
# fvcore tracing causes Triton compilation errors with Mamba's custom kernels
...
# Original: Crop(32, 4) -> 4/32 = 1/8. 224/8 = 28.
# Original: Cutout(1, 16) -> 16/32 = 1/2. 224/2 = 112.

transform_train = transforms.Compose([
    transforms.Resize((224, 224)), # Resize first to match model input
...
# ==========================================
print("\n" + "="*60)
print("🚀 Starting Training of Vision Mamba on CIFAR-100 (Matched Conditions)")
print("="*60 + "\n")

best_acc = 0.0

def save_checkpoint(model, acc, epoch, filename="vim_cifar100_best.pth"):
...
for epoch in range(CONFIG['epochs']):
    ...
    for inputs, targets in loop:
        ...
        loss.backward()
        # torch.nn.utils.clip_grad_norm_(model.parameters(), 1.0) # Disabled to match cifar.py
    ...
    if test_acc > best_acc:
        ...
        best_acc = test_acc
        save_checkpoint(model, best_acc, epoch+1, f"vim_cifar100_best_acc{best_acc:.2f}.pth")
