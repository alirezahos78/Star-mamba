...
sys.stdout = Logger(f'logs/log_efficient_vmamba_tiny_imagenet_scratch.txt')
...
CONFIG = {
    ...
    'warmup_epochs': 10,
    'repeated_aug': True,
    'img_size': 224, # Resizing to 224 (Tiny ImageNet is natively 64x64)
    'patch_size': 16,
    'workers': 8,
...
# ==========================================
# 3.1. Dataset Download & Setup
# ==========================================
def download_and_setup_tiny_imagenet(root_dir):
    ...
    if not os.path.exists(dataset_path):
        if not os.path.exists(filepath):
            print(f"⬇️ Downloading Tiny ImageNet from {url}...")
    ...
    # 3. Restructure Validation Set
    # The default validation set is flat (val/images/*.JPEG) with an annotation file.
    # We need to move them into subfolders (val/n01443537/images...) for ImageFolder.
    ...
    if os.path.exists(val_img_dir) and os.listdir(val_img_dir):
        ...
        with open(val_annotations_file, 'r') as f:
            for line in f:
                ...
                class_dir = os.path.join(dataset_path, 'val', class_id)
...
# Run setup
download_and_setup_tiny_imagenet(data_dir)

# Check if dataset exists
if not os.path.exists(train_dir):
    print(f"\n❌ ERROR: Tiny ImageNet Dataset not found at {train_dir}")


# Initialize Datasets using ImageFolder
...
# ==========================================
print("\n" + "="*60)
print("🚀 Starting Training of EfficientVMamba on Tiny ImageNet")
print("="*60 + "\n")

best_acc = 0.0

def save_checkpoint(model, acc, epoch, filename="efficient_vmamba_tiny_imagenet_best.pth"):
...
for epoch in range(CONFIG['epochs']):
    ...
    if test_acc > best_acc:
        ...
        best_acc = test_acc
        save_checkpoint(model, best_acc, epoch+1, f"efficient_vmamba_tiny_imagenet_best_acc{best_acc:.2f}.pth")
