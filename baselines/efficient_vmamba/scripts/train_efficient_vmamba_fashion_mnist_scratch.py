...
sys.stdout = Logger(f'logs/log_efficient_vmamba_fashion_mnist_scratch.txt')
...
# NOTE: Fashion-MNIST is 1 channel, so we set in_chans=1
model = EfficientVSSM(
    patch_size=CONFIG['patch_size'],
    in_chans=1, # Changed from 3 to 1 for grayscale
...
try:
    ...
    # Input dummy changed to 1 channel
    input_dummy = torch.randn(1, 1, CONFIG['img_size'], CONFIG['img_size']).to(CONFIG['device'])
...
# ==========================================
print("\n" + "="*60)
print("🚀 Starting Training of EfficientVMamba on Fashion-MNIST")
print("="*60 + "\n")

best_acc = 0.0

def save_checkpoint(model, acc, epoch, filename="efficient_vmamba_fashion_mnist_best.pth"):
...
for epoch in range(CONFIG['epochs']):
    ...
    if test_acc > best_acc:
        ...
        best_acc = test_acc
        save_checkpoint(model, best_acc, epoch+1, f"efficient_vmamba_fashion_mnist_best_acc{best_acc:.2f}.pth")
